...
@JsonIgnore // Post에서 Board의 대한 내용을 작성하기 않기 위해 사용. 없다면 순환오류 발생 (Board에도 Post가 있고 Post에도 Board가 있으므로)
...
GET http://localhost:8080/api/board/id/1 호출 시 PostList 확인 가능 > 글도 확인 가능

게시글 열람 : POST http://localhost:8080/api/post/view 호출 시 상위인 Board의 내용은 확인이 불가능, 오직 글만 확인 (@JsonIgnore 덕분에 순환오류 방지)
